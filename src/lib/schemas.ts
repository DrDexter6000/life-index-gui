...
// ── Agent Bridge schemas (CLI/L3 host-agent handoff) ──────────────────────

export const AgentBridgeConfiguredObjectSchema = z.object({
  configured: z.boolean(),
}).passthrough();

export const AgentBridgeAckSchema = z.object({
  data_exposure_ack: z.boolean(),
  required_for: z.array(z.string()).optional(),
}).passthrough();

export const AgentBridgeTokenSchema = z.object({
  configured: z.boolean(),
  source: z.string().optional(),
  persisted_in_config: z.boolean().optional(),
}).passthrough();

export const AgentBridgeProbeCheckSchema = z.record(z.string(), z.unknown());

export const AgentBridgeProbeResponseSchema = z.object({
  success: z.boolean(),
  schema_version: z.literal('m35.agent_bridge_probe.v0'),
  command: z.literal('agent-bridge probe'),
  source: z.string(),
  mode: z.string().optional(),
  transport: z.string().optional(),
  endpoint: AgentBridgeConfiguredObjectSchema,
  model: AgentBridgeConfiguredObjectSchema,
  ack: AgentBridgeAckSchema,
  token: AgentBridgeTokenSchema,
  checks: z.array(AgentBridgeProbeCheckSchema).default([]),
  sends_journal_evidence: z.literal(false),
  ready_to_send_evidence: z.boolean(),
}).passthrough().superRefine((payload, ctx) => {
  if ('scaffold' in payload) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'Agent Bridge probe must not include evidence scaffold fields.',
      path: ['scaffold'],
    });
  }
  if ('synthesis' in payload) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'Agent Bridge probe must not include synthesis fields.',
      path: ['synthesis'],
    });
  }
});

export const AgentBridgeGatewayHealthResponseSchema = z.object({
  running: z.boolean(),
  degraded: z.boolean().optional(),
  gateway_status: z.string().nullable().optional(),
...
const AgentBridgeAnswerSchema = z.object({
  mode: z.string().min(1),
  summary: z.string().optional(),
  reason: z.string().nullable().optional(),
  insights: z.array(InsightSchema).default([]),
  related_findings: z.array(z.unknown()).default([]),
  gap: z.string().nullable().default(null),
  explanation: z.string().nullable().default(null),
  what_was_found: z.array(z.unknown()).default([]),
  suggestions: z.array(z.string()).default([]),
}).passthrough();

const AgentBridgeScaffoldSchema = z.object({
...
export const AgentBridgeQueryResponseSchema = z.object({
  source: z.string(),
  query: z.string(),
  mode: z.string().min(1).optional(),
  reason: z.string().nullable().optional(),
  scaffold: AgentBridgeScaffoldSchema,
  evidence: z.array(EvidenceItemSchema).default([]),
  answer: AgentBridgeAnswerSchema.optional(),
  synthesis: z.string().nullable().default(null),
}).passthrough().superRefine((payload, ctx) => {
  // When answer is present, top-level mode must exist and match answer.mode
  if (payload.answer) {
    if (!payload.mode) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'mode is required when answer is present and must match answer.mode.',
        path: ['mode'],
      });
    } else if (payload.mode !== payload.answer.mode) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
  ...
  // UNGROUNDED responses promise the user no evidence was found.
...
// ── Agent Bridge streaming event schema ────────────────────────────────────

export const AgentBridgeStreamStatusEventSchema = z.object({
...
export const AgentBridgeStreamScaffoldEventSchema = z.object({
  type: z.literal('scaffold'),
  data: AgentBridgeScaffoldSchema,
});

export const AgentBridgeStreamEvidenceEventSchema = z.object({
  type: z.literal('evidence'),
  data: z.array(EvidenceItemSchema),
});

export const AgentBridgeStreamDeltaEventSchema = z.object({
...
const AgentBridgeStreamFinalDataSchema = AgentBridgeQueryResponseSchema.merge(
  z.object({
    schema_version: z.literal('m35.agent_bridge_query.v0').default('m35.agent_bridge_query.v0'),
    command: z.literal('agent-bridge query').default('agent-bridge query'),
  }),
).passthrough().superRefine((payload, ctx) => {
  // Final envelopes are the authoritative contract surface.
  if (payload.answer) {
    if (!payload.mode) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
  ...
  // Every evidence_ref in an insight must point to an item in evidence.
  const evidenceIds = new Set(payload.evidence.map((item) => item.id));
  for (const insight of payload.answer?.insights ?? []) {
    for (const ref of insight.evidence_refs) {
      if (!evidenceIds.has(ref)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: `insight references unknown evidence id "${ref}".`,
          path: ['answer', 'insights', 'evidence_refs'],
        });
      }
    }
  }
...
export const AgentBridgeStreamFinalEventSchema = z.object({
  type: z.literal('final'),
  data: AgentBridgeStreamFinalDataSchema,
});

export const AgentBridgeStreamErrorEventSchema = z.object({
...
export const AgentBridgeStreamEventSchema = z.union([
  AgentBridgeStreamStatusEventSchema,
  AgentBridgeStreamScaffoldEventSchema,
  AgentBridgeStreamEvidenceEventSchema,
  AgentBridgeStreamDeltaEventSchema,
  AgentBridgeStreamFinalEventSchema,
  AgentBridgeStreamErrorEventSchema,
]);
